...
import { Search, Plus, Filter, LayoutList, Download } from 'lucide-react';

const mockPricelists = [
  { id: 'PL-001', name: 'Public Pricelist (INR)', currency: 'INR', active: true },
  { id: 'PL-002', name: 'B2B Wholesale', currency: 'INR', active: true },
  { id: 'PL-003', name: 'Holiday Special', currency: 'INR', active: false },
];
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto min-h-[calc(100vh-3.5rem)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Pricelists</h1>
          <p className="text-sm text-gray-500 mt-1">Manage pricing rules and discounts.</p>
        </div>
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-gray-100">
            {mockPricelists.map((list) => (
              <tr key={list.id} className="hover:bg-gray-50/50 transition-colors cursor-pointer group">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider ${list.active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
